...
class NotificationAdapter : RecyclerView.Adapter<NotificationAdapter.VH>() {
    ...
    override fun onBindViewHolder(h: VH, pos: Int) {
        val n = items[pos]
        val initials = n.sender.split(" ").take(2).joinToString("") { it.take(1).uppercase() }.ifBlank { "?" }
        ...
        h.avatar.background.mutate().setTint(color and 0x00FFFFFF or 0x22000000)
    }
